...
| 2026-08-12 | M7 | C | 1 | 2 | 35 / +3401 −38 | ≈16m | ≈38m | healthy / bounded waits while Composer was active plus 2 expected file-delete approvals; final wait/get/read healthy; 0 orphaned panes | focused, evaluator, benchmark, `go test`, `go vet`, `go test -race`, OpenSpec strict 11/11 — PASS | 3 | Full review fixed restore-miss timing/counting, placeholder counters, safe formatting, custom-label cardinality, typed-nil options, race assertions and independent corpus ground truth; verification correction removed exported mutable entity lists and added JSON coverage/audit evidence. Specs synced and change archived. |
| 2026-08-12 | M8 | C | 1 | 2 | 26 / +1867 −5 | ≈3m | ≈30m | healthy / initial post-primary approval-review delay; single bounded liveness retry and all subsequent wait/get/read healthy; 0 orphaned panes | full dry-run: test, vet, race, external consumer, evaluator, exact license inventory/notices, 3×2s fuzz, 11 benchmarks; OpenSpec strict 12/12 — PASS | 2 | Full review completed third-party notices, corrected source-module distribution evidence, added explicit external Detect and removed pre-tag links; verification correction enforced exact dependency versions. One-line orchestrator fix isolated fuzz cache cleanup. Specs synced and change archived. |
| 2026-08-24 | PEB | Cursor-native | 3 | 2 | 81 / +6635 −101 vs `main` | 3 primary Composer 2.5 slices | C preflight failed (`HERDR_ENV` unset); no silent B fallback; Cursor-native `composer-worker` + Grok 4.6 high review | n/a (C not entered) / 0 orphaned panes | `go test`, `go vet`, `go test -race`, v1 markdown `-fail-on-regression`, generated contract+lifecycle, consumer, license, OpenSpec `--specs --strict` 13/13 — PASS | 12 | Core eval + mapping/lifecycle needed corrections (CLI exit, overmatch union, CSV indexes, smoke regen, HTTP timeout, lifecycle recipes/thresholds). Docs/CI slice APPROVE. FIAS/GAR disabled. External baseline diagnostic-only (`measured_commit` c351ecc). Specs synced and change archived. |
| 2026-08-25 | D1 | C | 1 | 2 | 11 / +523 −221 | ≈6m | ≈25m | healthy / 0 control timeouts / завершённый idle pane `w1:pJ` не закрыт: approval policy отклонила close / 1 orphaned pane | focused examples+consumer, links, `go test`, `go vet`, `go test -race`, OpenSpec strict 13/13 — PASS | 1 | Full review уточнил pre-release install, исправил misleading secret comment, вернул pure-Go/local positioning и улучшил русский перевод; post-correction read выявил оставшиеся языковые кальки. Specs synced, change archived. |
| 2026-08-26 | F1 | C | 1 | 1 | 13 product/spec / +154 −63; plus process artifacts | ≈4m | ≈14m | healthy / 0 control timeouts / recovery не требовался / 0 orphaned panes | Go 1.26.6 full dry-run, exact-minimum `govulncheck`, Windows+Darwin cross-build, OpenSpec strict 13/13 — PASS | 3 | Full review добавил exact-minimum proof вместо scan на произвольной newer toolchain, двуязычное описание двух gates и consolidated planned changelog. Один correction packet; specs synced, change archived, publication не выполнялась. |
| 2026-08-26 | F2 | C | 1 | 2 | 12 product/spec files; plus process artifacts | ≈10m | ≈25m | healthy / bounded work waits expired while Composer remained active; final get/read healthy / recovery не требовался / 0 orphaned panes | release helper 16/16, actionlint, Go 1.26.6 full dry-run, exact-minimum `govulncheck`, tag-peeling proof, OpenSpec strict 13/13 — PASS | 5 | Full review закрыл strict SemVer, literal changelog matching, tag collision и release-commit proof, а verification correction — trailing prerelease, exact planned header и annotated-tag peeling. Specs synced, change archived; tag/push/publication не выполнялись. |
