...
- [x] 4.8 Выполнить initial diagnostic run, проверить adapter alignment и disposition counts и сохранить безопасный aggregate baseline без raw values.
- [x] 4.9 После review initial baseline создать `prefilter-v1` threshold set с per-profile/entity/source boundaries и documented rationale.
...
- [x] 5.8 Добавить bounded generated/lifecycle smoke в `.github/workflows/ci.yml` без network и external cache.
- [x] 5.9 Добавить scheduled/workflow-dispatch external evaluation workflow с cache, pinned fetch, normalization и uploaded report artifact без auto-commit.
- [x] 5.10 Обновить `scripts/release-check.sh`, чтобы offline gates оставались network-free и readiness проверяла наличие external report metadata для exact release commit.
- [x] 5.11 Выполнить verification: `gofmt -w` для изменённых Go files, `go test ./...`, `go test -race ./...` и `go vet ./...`.
- [x] 5.12 Выполнить offline gates: `go run ./cmd/llmguard-eval -corpus ./testdata/evaluation/cases.jsonl -format markdown -fail-on-regression` и `./scripts/release-check.sh`.
- [x] 5.13 Воспроизвести pinned external evidence через `go run ./cmd/llmguard-eval-data -action fetch -manifest ./testdata/evaluation/manifests/redmadrobot-pii-benchmark-v1.json -cache ./.cache/llm-guard/evaluation`, затем `normalize` и `go run ./cmd/llmguard-eval -suite ./.cache/llm-guard/evaluation/normalized/redmadrobot-pii-benchmark-v1.jsonl -profile exposure -format json`.

## 6. Documentation

- [x] 6.1 Обновить README и package documentation: precision-oriented prefilter promise, supported/unsupported examples и явный отказ от high-recall DLP guarantee.
- [x] 6.2 Обновить `docs/known-limitations.md` границами PERSON, ADDRESS, checksum-invalid PII, unknown secrets и рекомендациями для high-risk integrations.
- [x] 6.3 Документировать suite v2 schema, mapping dispositions, contract/exposure formulas, zero-denominator rules и safe diagnostics policy.
- [x] 6.4 Документировать source manifests, cache/fetch/normalize commands, licenses, attribution и процедуру добавления нового benchmark source.
- [x] 6.5 Опубликовать initial safe baseline report с exact commands, revisions, mapping/threshold versions, environment и profile limitations.
- [x] 6.6 Обновить release checklist и MVP readiness matrix, разделив mandatory offline gates, external evidence freshness и diagnostic exposure results.
